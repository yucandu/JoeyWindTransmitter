...
static void pg_md5_encrypt(const char *password, char *salt, int salt_len, char *outbuf)
{
    ...
    *outbuf++ = '5';
    ...
    mbedtls_md5_update(&context, (uint8_t *)password, strlen(password));
    mbedtls_md5_update(&context, (uint8_t *)salt, salt_len);
    mbedtls_md5_finish( &context, sum);
    ...
    if (progmem) {
        ...
        if (buf) {
            if (buf_len <= l) {
        		...
        		if (client->connected()){n = client->write((const uint8_t *)Buffer, start - Buffer);}
        if (n != start - Buffer) return -1;
        if (buf && buf_len) {
            			if (client->connected()){n = client->write((const uint8_t *)buf, buf_len);}
